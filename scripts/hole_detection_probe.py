...
def detect_holes(arr, wafer, consensus):
    ...
    # Color stability across frames: use average RGB temporal std
    color_std = arr.std(axis=0).mean(axis=2)

    # If scaffold estimate is empty or too small, use full image as fallback
    ...
    # Apply scaffold constraint and margin to avoid clipping hole boundaries
    ...
    # Identify candidate hole centers from consensus but restricted to stable zone
    ...
    # Estimate hole radius from stable area regions; fallback to center spacing
    ...
    # Enforce regular grid: keep points with consistent neighbor spacing
    if len(centers) > 1:
...
def run_probe(video_path: str, out_dir: str):
    ...
    ax.set_title(f"consensus candidates ({len(candidates)})")
    ax.axis("off")
    fig.tight_layout()
